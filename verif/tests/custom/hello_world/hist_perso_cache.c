...
void print_dec_nolf(uint64_t val)
{
    char buf[21];
    int i = 0;
...
static inline void Init_cache_prime (void *addr,size_t other_set) // set le cache pour avoir la bonne mesure prime probe 
{
 size_t pset = (((size_t)addr) >> 4) & 0xFF; // Init le set pour le cache 
...
static inline void prime_probe(void *addr,size_t other_set) // set le cache pour avoir la bonne mesure prime probe 
{
 size_t pset = (((size_t)addr) >> 4) & 0xFF; // Init le set pour le cache 
...
static inline void victime(void *addr,size_t other_set)
{
 size_t pset = (((size_t)addr) >> 4) & 0xFF; // Init le set pour le cache 
...
size_t measure_access_time(void *address,size_t set) {
 uint64_t x = rdcycle();
 prime_probe(address,set);
 uint64_t y = rdcycle();
...
uint64_t hit_event[256][MEASUREMENTS];
uint64_t hit_event_ref[256][MEASUREMENTS];

uint64_t ref_miss[256][MEASUREMENTS];
uint64_t victim_miss[256][MEASUREMENTS];
uint64_t victim_evict[256][MEASUREMENTS];
uint64_t probe_miss[256][MEASUREMENTS];
uint64_t cumul_refTab[256][MEASUREMENTS];
uint64_t cumul_primeTab[256][MEASUREMENTS];
...
void measure_prime_ref(void *address, size_t *histogram, size_t number_of_measurements) {
 ...
 prime_probe(address,f); 

 uint64_t m0 = read_csr(CSR_HPMCOUNTER6);
 uint64_t h1 = lecture_hit();

 size_t prime = measure_access_time(address,f); 
 ...
 hit_event_ref[f][i] = h2-h1;
 ref_miss[f][i] = m1-m0;
 cumul_refTab[f][i] = prime;
...
void measure_prime_probe(void *address, size_t *histogram, size_t number_of_measurements) {
 ...
 Init_cache_prime(address,f); // re remplace les lignes parasites 
 //prime_probe(address); 
 ...
 size_t probe = measure_access_time(address,f);
 ...
 hit_event[f][i] = (h2-h1); 
 victim_miss[f][i] = (m1-m0); 
 victim_evict[f][i]= (e1-e0);
 probe_miss[f][i] =(e2-e1) ;
 cumul_primeTab[f][i] = probe;
...
set_lecture_hit();
 ...
 measure_prime_probe(address, miss_histogram, MEASUREMENTS);
    ...
    print_dec(cumul_ref / (256 * MEASUREMENTS));
    ...
    print_dec(cumul_prime / (256 * MEASUREMENTS));
 /*
 ...
 (unsigned long)probe_miss[i]);
    ...
    for (size_t f = 0; f < 256; f++) {
        ...
        for (size_t i = 0; i < MEASUREMENTS; i++) {
            ...
            print_str(" ref=");
            print_dec_nolf(cumul_refTab[f][i]);

            print_str(" prime=");
            print_dec_nolf(cumul_primeTab[f][i]);

            print_str(" miss=");
            print_dec_nolf(probe_miss[f][i]);

            uart_putc('\n');

            //print_str("hit_event_ref = ");
            //print_dec(hit_event_ref[f][i]);

            //print_str("hit_event probe = ");
            //print_dec(hit_event[f][i]);
